...
export default function NoticeBulletinBoard() {
    ...
    useEffect(() => {    
        const fetchData = async () => {
            try {
                if (cookies.get('page')) {
                    const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/boards/NOTICE/list?page=${cookies.get('page')}`);    
                    setPosts(response.data);
                    setPage(cookies.get('page'));
                }
                else {
                    const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/boards/NOTICE/list?page=1`);
                    setPosts(response.data);
                    cookies.set('page', 1);
                }
                setPostsLoaded(true);
    ...
    const goToNoticeBulletinBoardPageWriting = () => {
        if (cookies.get('accessToken') && cookies.get('refreshToken') && decodeURIComponent(cookies.get('email')) === 'darkest0722@gmail.com') {
            ...
            Swal.fire({
                icon: "info",
                title: '공지 게시판의 게시글은 관리자만 작성 가능합니다.',
                showCancelButton: false
            });
        }
    }
