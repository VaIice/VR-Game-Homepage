...
export default function ReportBulletinBoardPage(bno) {
    ...
    useEffect(() => {    
        const fetchData = async () => {
            if (cookies.get('secret') === 1) {
                try {
                    const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/boards/REPORT/${bno.bno}/withImages`, {
                        headers: {
                            'Authorization': `Bearer ${cookies.get('accessToken')}`,
                        }
                    });
                    const responseComment = await axios.get(`${process.env.REACT_APP_SERVER_URL}/replies/REPORT/list/${bno.bno}`);
    ...
    const onClickCommentRemoveButton = (rno) => {
        ...
        .then(async (res) => {
            if (res.isConfirmed) {
                try {
                    const response = await axios.delete(`${process.env.REACT_APP_SERVER_URL}/replies/REPORT/${rno}`, {
                        headers: {
                            'Authorization': `Bearer ${cookies.get('accessToken')}`}
                    });
                    const responseComment = await axios.get(`${process.env.REACT_APP_SERVER_URL}/replies/REPORT/list/${bno.bno}`);
    ...
    const onClickRemoveButton = async () => {
        ...
        .then(async (res) => {
            if (res.isConfirmed) {
                try {
                    const response = await axios.delete(`${process.env.REACT_APP_SERVER_URL}/boards/remove/REPORT/${bnum}`, {
                        headers: {
                            'Authorization': `Bearer ${cookies.get('accessToken')}`}
                    });
                    goToReportBulletinBoard();
                    ...
                    Swal.fire({
                        icon: "info",
                        title: '해당 게시글은 관리자와 작성자만 삭제 가능합니다.',
                        showCancelButton: false
                    });
                }
            }
    ...
    const onClickCommentEnrollButton = async () => {
        try {
            const response = await axios.post(`${process.env.REACT_APP_SERVER_URL}/replies/REPORT/register`, {bno: bnum, replyText: comment}, {
                headers: {
                    'Authorization': `Bearer ${cookies.get('accessToken')}`}
            });
            const responseComment = await axios.get(`${process.env.REACT_APP_SERVER_URL}/replies/REPORT/list/${bno.bno}`);
    ...
    const onClickCommentModifyButton = async () => {
        try {
            const response = await axios.put(`${process.env.REACT_APP_SERVER_URL}/replies/REPORT/${modifyCommentRno}`,
            {
    ...
    const handlePage = async (page) => {
        try {
            const SERVER_URL_COMMENT_PAGE = `${process.env.REACT_APP_SERVER_URL}/replies/REPORT/list/${bnum}?page=${page}`;
            const fetchData = async () => {
    ...
    const goToReportBulletinBoardPageModifyWriting = () => {
        const decodedEmail = decodeURIComponent(cookies.get('email'));
        if (decodedEmail === writer && cookies.get('accessToken')) {
            navigate(`/ReportBulletinBoardPageModifyWriting/${bno.bno}`)    
        }
        else {
            Swal.fire({
                icon: "info",
                title: '해당 게시글은 관리자와 작성자만 수정 가능합니다.',
                showCancelButton: false
            });
        }
    }
            ...
            <div className="contentWrap123">
                <div className = "BulletinBoardWritingUpper">
                    ...
                    <div className="BulletinBoardWritingUpperRight">
                        <button className="BulletinBoardImageButton123" onClick={goToReportBulletinBoard}>
                ...
                <div className = "BulletinBoardWritingMiddle">
                    ...
                    <div className = "BulletinBoardPageContentsWrap">
                        ...
                        <div className="BulletinBoardPageContentsBottom">
                            <button className="BulletinBoardPageModifyButton" onClick={goToReportBulletinBoardPageModifyWriting}>
                                수정
                            </button>
